...
Tensor::Tensor()
    : impl{std::make_unique<TensorImpl>(*this)}, storagePtr{std::make_shared<Storage>()} {}
...
int64_t Tensor::TensorImpl::stride(int64_t dim) const {
  ...
  if (dim < 0 || stride_.size() <= dim) {
    // TODO: how to avoid this stupid concatenation?
    throw std::invalid_argument(std::string("Wrong dimension is provided: dim=") +
                                std::to_string(dim));
  }
  // TODO: implement Stride::operator[]
  return stride_[dim];
}

const std::vector<int64_t>& Tensor::TensorImpl::shape() const noexcept { return shape_; }
...
const std::vector<int64_t>& Tensor::shape() const noexcept { return impl->shape(); }
