...
// Gaussian PDF in point x: return -0.5 * (2 * M_PI * std_ * std_).log() - (x - mean_).square() / (2
// * std_ * std_));
      ...
      public:
        ...
        MatrixXd operator()(const MatrixXd& x) const {
          ...
          for (int ind = 0; ind < labelsCnt_; ind++) {
            ...
            auto preLogScore =
                (((x.array().rowwise() - mean_.row(ind).array()).square()).rowwise() /
                 (-2.0 * stdRowSq))
                    .rowwise() -
    ...
    void train(const MatrixXd& features, const MatrixXd& target) {
      ...
      for (auto label : classLabels_) {
        ...
        featureStd.row(label) =
            ((selectedRows.rowwise() - featureMeans.row(label)).array().square().colwise().sum() /
             divide_by);
...
void GaussianNaiveBayes::train(const MatrixXd& features, const MatrixXd& target) {
