...
// Gaussian PDF in point x: return -0.5 * (2 * M_PI * std_ * std_).log() - (x -
// mean_).square() / (2
// * std_ * std_));
class GaussianNaiveBayes::Impl {
  ...
  public:
    ...
    GaussianPDF(const MatrixXd& mean, const MatrixXd& stddev,
                const int& labelsCnt)
    ...
    MatrixXd operator()(const MatrixXd& x) const {
      if (!(x.cols() == mean_.cols() && x.cols() == stdSquared_.cols())) {
        throw std::invalid_argument(
            "Wrong data for GaussianPDF: features number don't match.\n");
      }
      ...
      for (int ind = 0; ind < labelsCnt_; ind++) {
        ...
        auto preLogScore =
            (((x.array().rowwise() - mean_.row(ind).array()).square())
                 .rowwise() /
             (-2.0 * stdRowSq))
                .rowwise() -
...
public:
  Impl()
      : isFitted_{false}, labelsCnt_{}, labelsLogProbas_{}, gaussianPDF_{} {};
  ...
  void train(const MatrixXd& features, const MatrixXd& target) {
    ...
    for (auto label : classLabels_) {
      ...
      featureStd.row(label) =
          ((selectedRows.rowwise() - featureMeans.row(label))
               .array()
               .square()
               .colwise()
               .sum() /
...
void GaussianNaiveBayes::train(const MatrixXd& features,
                               const MatrixXd& target) {
  pImpl_->train(features, target);
}

MatrixXd GaussianNaiveBayes::predict(const MatrixXd& features) const {
  return pImpl_->predict(features);
}

MatrixXd GaussianNaiveBayes::predict_proba(const MatrixXd& features) const {
  return pImpl_->predict_proba(features);
}
